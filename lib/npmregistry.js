...
function normalize(pkg, isv) {
  ...
  npkg.bugs = pkg.bugs;
  // The CommonJS specification does not define an `author` property, instead
  // having a convention where the author is the first element in the
  // `contributors` property.  `author` is an extension used within npm and is
  // preserved here for convienience.
  if (pkg.author) {
